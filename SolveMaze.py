...
#To return all important details of the solution
class Result(object):
    def __init__(self,maze,nodes_visited,path_length,solvable):
        self.maze=maze
...
def DFS(maze):
    ...
    while(DFSstack):
        ...
        if(s.goal_State(DFSmaze)):
            ...
            for p in path:
                DFSmaze[p.x][p.y]=5
            return Result(DFSmaze,number_nodes_visited,len(path),1)
    ...
    return Result(DFSmaze,number_nodes_visited,0,0)
...
def BFS(maze):
    ...
    while(BFSQueue):
        ...
        if(s.goal_State(BFSmaze)):
            ...
            for p in path:
                BFSmaze[p.x][p.y]=5
            return Result(BFSmaze,number_nodes_visited,len(path),1)
        for neighbor in (s.getNeighbors(nodeMaze)):
            neighbor.parent = s
            neighbor.visited = True
    ...
    return Result(BFSmaze,number_nodes_visited,0,0)
...
def astar(maze,heuristic):
    ...
    while len(open_list) > 0:
        ...
        if curr_node == end_node:
            ...
            return Result(a_maze,number_nodes_visited,len(path),1)
        children = []
    ...
    return Result(a_maze,number_nodes_visited,0,0)
